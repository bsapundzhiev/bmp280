...
static int bmp280_i2c_device_probe(struct i2c_adapter *i2c_adapter,
				   unsigned short adapter)
{
	struct i2c_client *i2c_device = i2c_new_scanned_device(i2c_adapter,
	...
	pr_info("Found device: %s @ i2c-%d addr: 0x%x\n",
		i2c_device->name, adapter, i2c_device->addr);

	return 0;
}

static int bmp280_i2c_devices_probe(void)
{
	unsigned short adapter = 0;
	pr_info("Probing i2c adapters: [%d-%d]\n", adapter, I2C_ADAPTERS_MAX);

	for (adapter = adapter; adapter <= I2C_ADAPTERS_MAX; adapter++) {

		struct i2c_adapter *i2c_adapter = i2c_get_adapter(adapter);

		if (i2c_adapter) {
			bmp280_i2c_device_probe(i2c_adapter, adapter);
		}
	}
...
{
	return bmp280_i2c_devices_probe();
}
